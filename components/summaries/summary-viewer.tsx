...
const SectionTitle = ({title} : {title : string}) => {
    return (
        <div className="flex flex-col gap-2 mb-6 sticky top-0 pt-2 pb-4 bg-background/80 backdrop-blur-xs z-10 ">
            <h2 className="text-2xl lg:text-3xl font-bold text-center flex items-center justify-center gap-2 "> {title}</h2>
        </div>
    )
}
...
export function SummaryViewer({ summary }: { summary: string }) {
  ...
  return (
    <Card className="relative px-2 h-[500px] sm:h-[600px] lg:h-[700px] w-full xl:w-[600px] overflow-hidden bg-linear-to-r from-background vai-background/95 to-rose-500/5 background-blur-lg shadow-2xl rounded-3xl border border-rose-500/10">
